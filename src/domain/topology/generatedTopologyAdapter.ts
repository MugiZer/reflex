...
export type GeneratedTopologyAdapter = Readonly<{
  ...
  qualificationCases: Readonly<{ reference: Readonly<{ caseId: string; parameters: Readonly<Record<string, number>>; expectedEffectiveUValueWPerM2K: number; toleranceWPerM2K: number }>; sensitivity: Readonly<{ caseId: string; parameters: Readonly<Record<string, number>>; direction: "increases" | "decreases" }> }>;
...
function isRecipeShape(value: JsonValue, module: { id: string; version: string }): boolean { if (!isRecord(value)) return false; const recipe = value as Record<string, unknown>; return recipe.schemaVersion === "1.0.0-draft" && isRecord(recipe.topologyModule) && recipe.topologyModule.id === module.id && recipe.topologyModule.version === module.version && Array.isArray(recipe.layers) && recipe.layers.length > 0 && Array.isArray(recipe.rows) && recipe.rows.length > 0 && isRecord(recipe.boundaries); }
