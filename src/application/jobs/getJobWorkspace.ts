...
export type JobWorkspaceViewModel = { job: JobRecord; topologyReviews: ReturnType<JobRepository["listTopologyReviews"]>; pilotRuns: ReturnType<JobRepository["listTopologyPilotRuns"]>; pilotEvents: ReturnType<NonNullable<JobRepository["listTopologyPilotEvents"]>>; topologyOpportunities: JobTopologyOpportunityView[]; review: (JobReviewState & { context: ReviewContextViewModel; projection: ActionReadyReviewProjection }) | null; architectActions: ReturnType<typeof buildArchitectActionViewModel>; materialLibrary: typeof defaultMaterialLibraryV1; thermalTreatmentCards: ReturnType<typeof buildThermalTreatmentCardModel> };
export async function getJobWorkspace(command: { jobs: JobRepository; evidence: JobWorkspaceEvidenceLoader; topologyIntegrity: TopologyResultIntegrityVerifier; jobId: string; targetUValueWPerM2K: number | null; thermalTreatmentRegistry?: ThermalTreatmentFamilyRegistry }): Promise<JobWorkspaceViewModel | null> {
  ...
  const pilotRuns = command.jobs.listTopologyPilotRuns(command.jobId);
  const pilotEvents = command.jobs.listTopologyPilotEvents?.(command.jobId) ?? [];
  ...
  return { job, topologyReviews, pilotRuns, pilotEvents, topologyOpportunities, review, architectActions: buildArchitectActionViewModel({ jobId: command.jobId, jobStatus: job.jobStatus, calculationInputEvidence: evidence, requestedInputs: reviewState?.requestedInputs ?? [], activeRevision, target: command.targetUValueWPerM2K === null ? null : { maxUValueWPerM2K: command.targetUValueWPerM2K, label: "Working project target" }, materialLibrary: defaultMaterialLibraryV1 }), materialLibrary: defaultMaterialLibraryV1, thermalTreatmentCards: buildThermalTreatmentCardModel({ suggestions: detectThermalTreatmentOpportunities({ calculationInputEvidence: evidence, registry: thermalTreatmentRegistry }).suggestions, registry: thermalTreatmentRegistry, activeRevision, assemblyGroupIdsForSuggestion: (suggestion) => [...new Set(evidence.filter((item) => suggestion.affectedElementStepIds.includes(item.elementStepId)).map(assemblyGroupIdForEvidence))] }) };
}
