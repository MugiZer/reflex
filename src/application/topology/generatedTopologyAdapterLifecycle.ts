...
export async function rehydrateGeneratedTopologyAdapterRegistry(command: { manifests: GeneratedTopologyAdapterManifestStore; registry: GeneratedTopologyAdapterRegistry; bundle: TopologyBundleIdentity; onDiagnostic?: (diagnostic: DurableAdapterDiagnostic) => Promise<void> | void }): Promise<Readonly<{ outcome: "restart"; loaded: number; diagnostics: readonly DurableAdapterDiagnostic[] }>> {
  ...
  for (const entry of await command.manifests.scan()) {
    ...
    if (disabled) { diagnostics.push({ outcome: "disabled", path: entry.path, message: "Adapter is disabled." }); continue; }
    if (!manifestEligible(entry.manifest, command.bundle)) { diagnostics.push({ outcome: "incompatibility", path: entry.path, message: "Adapter receipt or dependencies are not eligible for this production bundle." }); continue; }
    eligible.push({ adapterHash: entry.manifest.adapterHash, adapter: entry.manifest.adapter });
  }
  for (const diagnostic of diagnostics) { try { await command.manifests.recordDiagnostic(diagnostic); } catch {} try { await command.onDiagnostic?.(diagnostic); } catch {} }
  command.registry.replace(eligible);
