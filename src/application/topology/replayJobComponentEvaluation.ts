...
export function replayJobComponentEvaluation(command:{jobId:string;evaluationId:string;patternId:string;patternVersion:string;jobs:JobRepository;componentEvaluations:ComponentEvaluationRepository;patterns:readonly ComponentPattern[]}){
  const original=command.componentEvaluations.getByEvaluationId(command.evaluationId);
  ...
  if(!pattern)throw new ReplayComponentEvaluationError("promoted_pattern_unavailable","Promoted component pattern is unavailable.");
  const replay=replayUnresolvedOccurrence({original,pattern,promotedAt:pattern.promotedAt??original.evaluation.createdAt});
  command.componentEvaluations.append(replay);
  return replay;
}
