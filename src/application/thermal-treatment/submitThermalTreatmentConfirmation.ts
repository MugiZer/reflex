...
export async function submitThermalTreatmentConfirmation(command: { jobId: string; body: unknown; jobs: JobRepository; deps: ProcessIfcJobDeps; registry: ThermalTreatmentFamilyRegistry; worker: ThermalTreatmentCalculationWorker }) {
  ...
  if (job.jobStatus !== "completed") throw thermalConflict("Thermal Treatment can be confirmed after the Assembly Group has an active calculation.");
  ...
  if (!evidence || !activeRevision) throw thermalConflict("Stored calculation evidence or active Revision is missing.");
  ...
  if (!suggestion) throw thermalConflict("Thermal Treatment suggestion is stale, unknown, or no longer matches the stored IFC evidence. Refresh the Review and try again.");
  ...
  if (!affectedAssemblyGroupIds.includes(submission.assemblyGroupId)) throw thermalConflict("The selected Assembly Group is outside the unchanged Thermal Construction Signature.");
  const family = command.registry.findByIdentity(suggestion.family);
  if (!family) throw thermalConflict("The selected Thermal Treatment family is not registered.");
  const allowed = new Set(family.packs.knowledgePack.parameters.map((input) => input.key));
  if (Object.keys(submission.inputs).some((key) => !allowed.has(key))) throw thermalInput("The confirmation includes an unknown Thermal Treatment parameter.");
  ...
  if (validationIssues.length) throw thermalInput(validationIssues.map((issue) => issue.message).join(" "));
  ...
  try {
    ...
    if (job.activeRevisionId) await command.deps.completedJobPublication.restoreActiveRevision(command.jobId, job.activeRevisionId);
    throw error;
  }
...
function parseSubmission(body: unknown): { suggestionId: string; thermalConstructionSignature: string; familyId: string; familyVersion: string; assemblyGroupId: string; inputs: Record<string, ThermalTreatmentInputValue> } {
  if (!isRecord(body)) throw thermalInput("Expected a Thermal Treatment confirmation object.");
  for (const key of ["suggestionId", "thermalConstructionSignature", "familyId", "familyVersion", "assemblyGroupId"]) if (typeof body[key] !== "string" || body[key].trim() === "") throw thermalInput(`${key} is required.`);
  if (!isRecord(body.inputs)) throw thermalInput("inputs must be an object of architect-owned values.");
  for (const value of Object.values(body.inputs)) if (!(typeof value === "string" || typeof value === "number" || typeof value === "boolean" || value === null)) throw thermalInput("Thermal Treatment input values must be strings, numbers, booleans, or null.");
...
function validateEnvelope(envelope: Readonly<Record<string, { minimum?: number; maximum?: number; allowedValues?: readonly ThermalTreatmentInputValue[] }>>, values: Record<string, ThermalTreatmentInputValue>) {
  for (const [key, value] of Object.entries(values)) { const bounds = envelope[key]; if (!bounds) continue; if (typeof value === "number" && ((bounds.minimum !== undefined && value < bounds.minimum) || (bounds.maximum !== undefined && value > bounds.maximum))) throw thermalInput(`${key} is outside the supported validation envelope.`); if (bounds.allowedValues && !bounds.allowedValues.includes(value)) throw thermalInput(`${key} is outside the supported validation envelope.`); }
}
function isRecord(value: unknown): value is Record<string, unknown> { return typeof value === "object" && value !== null && !Array.isArray(value); }
function thermalInput(message: string): ApplicationFailure { return new ApplicationFailure("invalid_input", "invalid_thermal_treatment", message); }
function thermalConflict(message: string): ApplicationFailure { return new ApplicationFailure("conflict", "thermal_treatment_conflict", message); }
