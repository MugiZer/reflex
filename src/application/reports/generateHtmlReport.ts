...
function renderAssembly(snapshot: CalculationSnapshot, index: number): string {
  ...
  return "<article class=\"assembly-view" + active + "\" data-assembly-index=\"" + index + "\">" +
    ...
    renderThermalTreatmentResult(thermalTreatment) +
...
function renderThermalTreatmentResult(thermalTreatment: CalculationSnapshot["thermalTreatment"]): string {
  if (!thermalTreatment) return "";
  const verified = thermalTreatment.trustState === "verified";
  const convergence = thermalTreatment.numericalResult?.convergence;
  return "<section class=\"material-values thermal-treatment-result " + (verified ? "verified" : "preliminary") + "\"><div class=\"section-title\"><div><span class=\"eyebrow\">Thermal Treatment</span><h3>" + (verified ? "Verified results" : "Preliminary calculations — not verified") + "</h3></div><span>Design decision support only</span></div><p><b>Family:</b> " + escapeHtml(thermalTreatment.selection.familyId + " v" + thermalTreatment.selection.familyVersion) + "</p><p><b>Effective versus layer-only U-value:</b> " + thermalTreatment.effectiveUValueWPerM2K.toFixed(3) + " versus " + thermalTreatment.baselineUValueWPerM2K.toFixed(3) + " W/m2K</p><h3>Confirmed inputs</h3>" + renderList(Object.entries(thermalTreatment.confirmedInputs).map(([key, value]) => key + ": " + String(value)), "No confirmed inputs.") + "<h3>Assumptions and missing inputs</h3>" + renderList([...thermalTreatment.assumptions, ...thermalTreatment.trustReasons.map((reason) => reason.message), ...thermalTreatment.actionsRequiredForVerification], "No unresolved inputs.") + "<h3>Pack and worker versions</h3><p>Adapter " + escapeHtml(thermalTreatment.packVersions.codeAdapterVersion) + " / Knowledge " + escapeHtml(thermalTreatment.packVersions.knowledgePackVersion) + " / Validation " + escapeHtml(thermalTreatment.packVersions.validationPackVersion) + " / " + escapeHtml(thermalTreatment.worker.workerId + " v" + thermalTreatment.worker.workerVersion) + "</p>" + renderQualification(thermalTreatment) + (convergence ? "<h3>Convergence evidence</h3><p>" + (convergence.passed ? "Passed" : "Did not pass") + "; refinements: " + convergence.refinementLevels + "; relative difference: " + (convergence.relativeDifference === null ? "not available" : convergence.relativeDifference.toFixed(6)) + ".</p>" : "") + "<h3>Solver artifacts</h3>" + renderList([...(thermalTreatment.artifactReferences ?? [])], "No solver artifacts were recorded.") + "</section>";
}
function renderQualification(thermalTreatment: NonNullable<CalculationSnapshot["thermalTreatment"]>): string {
  if (!thermalTreatment.qualification || !thermalTreatment.generation) return "";
  const qualification = thermalTreatment.qualification;
  return "<h3>Qualification</h3><p>Decision: " + escapeHtml(qualification.decision.toUpperCase()) + "; dataset " + escapeHtml(thermalTreatment.generation.dataset.datasetId + " v" + thermalTreatment.generation.dataset.datasetVersion) + "; hash " + escapeHtml(thermalTreatment.generation.dataset.contentHash) + "; adapter " + escapeHtml(qualification.codeAdapterVersion) + "; worker " + escapeHtml(qualification.worker ? qualification.worker.workerId + " v" + qualification.worker.workerVersion : "not run") + ".</p><p>Oracle " + escapeHtml(qualification.oracle.oracleId + " v" + qualification.oracle.oracleVersion) + "; hash " + escapeHtml(qualification.oracle.contentHash) + "; source " + escapeHtml(qualification.oracle.sourceCitation) + ".</p><h4>Supported envelope</h4>" + renderList(Object.entries(qualification.supportedParameterEnvelope).map(([key, bounds]) => key + ": " + JSON.stringify(bounds)), "No supported envelope was recorded.") + "<h4>Reference cases</h4>" + renderList(qualification.referenceCases.map((referenceCase) => referenceCase.caseId + ": " + (referenceCase.passed ? "passed" : "failed") + " (expected " + referenceCase.expectedEffectiveUValueWPerM2K + ", actual " + (referenceCase.actualEffectiveUValueWPerM2K ?? "unavailable") + ", tolerance " + referenceCase.toleranceWPerM2K + ")"), "No qualification reference cases were recorded.") + "<h4>Qualification notes</h4>" + renderList(qualification.reasons, "No qualification notes.");
}
