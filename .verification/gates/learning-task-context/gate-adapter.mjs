...
const gate = process.env.PROTECTED_VERIFIER_GATE;
const requiredCases = JSON.parse(process.env.PROTECTED_VERIFIER_CASES ?? "[]");

if (!evidenceRoot || !candidate || !gate) throw new Error("Protected verifier environment is incomplete");

const probe = await run(candidate, ["npx", "tsx", join(gate, "gate-probe.ts"), candidate]);
let observations;
try {
  ...
  observations = Object.fromEntries(requiredCases.map((id) => [id, {
    status: "FAIL",
    observation: probe.stderr || probe.stdout || "Task-context probe produced no result",
  }]));
}
