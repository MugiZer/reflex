...
describe("ambiguous generated topology family fit", () => {
  it("offers only plausible contracts and authorizes the proposed candidate by fixed gates", () => {
    ...
    const left = adapter("left"), right = adapter("right"); registry.add(generatedTopologyAdapterHash(left), left, receipt(left)); registry.add(generatedTopologyAdapterHash(right), right, receipt(right));
  ...
  it("calls fit only for ambiguous candidates and rejects a confident outside-envelope proposal", async () => {
    const registry = new GeneratedTopologyAdapterRegistry(); const left = adapter("left"), right = adapter("right"); registry.add(generatedTopologyAdapterHash(left), left, receipt(left)); registry.add(generatedTopologyAdapterHash(right), right, receipt(right));
    ...
    expect(saved.find((item) => item.fitDecision)).toMatchObject({ fitDecision: { canonicalSignature: expect.any(String), candidateIdentities: expect.any(Array), finalDisposition: "authorized", gates: { qualification: true } } });
    ...
    expect(saved.find((item) => item.fitDecision && item.result.correlationId === "fit-2")).toMatchObject({ fitDecision: { finalDisposition: "generation", gates: { contract: true, envelope: false, recipe: true, dependencies: true, qualification: true } } });
  });

  it("retries provider infrastructure failures under one fit correlation without correction cycles", async () => {
    const registry = new GeneratedTopologyAdapterRegistry(); const left = adapter("left"), right = adapter("right"); registry.add(generatedTopologyAdapterHash(left), left, receipt(left)); registry.add(generatedTopologyAdapterHash(right), right, receipt(right));
    const saved: any[] = []; let calls = 0;
    const agent: AmbiguousFamilyFitAgent = {
      model: "fixture", skillVersion: "fit/v1", retry: { maxAttempts: 2, backoffMs: 0 },
      attempts: { append: async (attempt: any) => { saved.push(attempt); }, listByCorrelationId: async (correlationId) => saved.filter((attempt) => attempt.result.correlationId === correlationId) },
      provider: { execute: async (request: any) => {
        calls += 1;
        if (calls === 1) return { kind: "retryable_infrastructure_failure", reason: "temporary network", attemptEvidence: { provider: "fixture", model: request.model, correlationId: request.correlationId, startedAt: `2026-08-18T00:00:0${calls}.000Z`, durationMs: 1, outcome: "retryable_infrastructure_failure", safeUsage: null } };
        return { kind: "completed", output: { candidateIdentity: fitCandidateContracts({ answers: answers(), bundle: PROVEN_TOPOLOGY_BUNDLE, registry })[0]!.identity, confidence: "high", comparison: [], reasons: ["recovered"] }, attemptEvidence: { provider: "fixture", model: request.model, correlationId: request.correlationId, startedAt: `2026-08-18T00:00:0${calls}.000Z`, durationMs: 1, outcome: "completed", safeUsage: null } };
      } },
    };
    await expect(attemptAmbiguousGeneratedTopologyFamilyFit({ answers: answers(), bundle: PROVEN_TOPOLOGY_BUNDLE, registry, agent, canonicalEvidenceReference: "evidence:retry", correlationId: "fit-retry", deadline: new Date(Date.now() + 1_000) })).resolves.not.toBeNull();
    expect(calls).toBe(2);
    expect(await agent.attempts.listByCorrelationId("fit-retry")).toHaveLength(3);
    expect(saved.find((item) => item.fitDecision && item.result.correlationId === "fit-retry")).toMatchObject({ fitDecision: { finalDisposition: "authorized", skillVersion: "fit/v1" } });
  });
});
...
function receipt(adapter: GeneratedTopologyAdapter): GeneratedTopologyQualificationReceipt { const adapterHash = generatedTopologyAdapterHash(adapter); const gate = (gateId: any) => ({ gateId, selectedCases: [gateId], passedCases: [gateId], failedCases: [], unexecutedCases: [], fixtureIdentity: "fixture", oracleIdentity: null, adapterHash, dependencyIdentities: adapter.dependencies, command: "fixture", durationMs: 0, testedRevision: "fixture" }); return { schema: "generated-topology-adapter-qualification-receipt/v1", decision: "GO", adapterHash, recipeHash: null, worker: { executable: "fixture", runtimeHash: adapter.dependencies.runtimeHash }, compilerVersion: adapter.dependencies.compilerVersion, primitiveRegistryHash: adapter.dependencies.primitiveRegistryHash, materialPackHash: adapter.dependencies.materialPackHash, boundaryVersion: adapter.dependencies.boundaryVersion, gates: [gate("P3-contract-geometry"), gate("P6-worker"), gate("P3-independent-reference"), gate("P6-envelope-sensitivity")], qualifiedAt: "2026-08-18T00:00:00.000Z" }; }
